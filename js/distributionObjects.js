...
function distributionParameter(
  name,
  label,
  range,
  interactRange,
...
function createElement(fstart, frangeLo, frangeHi) {
  ...
  switch (this.type) {
    ...
    case "continuous":
      ...
      if (log) {
        var sliderStart =
          (sliderMax * (Math.log(start) - Math.log(limits[0]))) /
          (Math.log(limits[1]) - Math.log(limits[0]));
      } else {
        var sliderStart =
          (sliderMax * (start - limits[0])) / (limits[1] - limits[0]);
      }
      break;
  }

  $("#params_slider_" + name).slider({
    min: sliderMin,
    max: sliderMax,
    value: sliderStart,
  });
...
function distributionParametrization(
  ptznName,
  params,
  limits,
  interactLimits,
...
function updatePlot(ptznNum, name, ui) {
  ...
  for (; i < npars; i++) {
    ...
    sliderValue =
      name == par.name
        ? ui.value
        : $("#params_slider_" + par.name).slider("value");
    ...
    switch (par.type) {
      case "discrete":
        ...
        if (name == par.name)
          $("#params_indicator_" + par.name).val(
            round(sliderValue, roundDigits)
          );
        break;
      ...
      case "continuous":
        ...
        if (par.interactLog) {
          parValue = Math.exp(
            sliderFrac * (Math.log(limits[1]) - Math.log(limits[0])) +
              Math.log(limits[0])
          );
        } else {
        ...
        args.push(parValue);
        if (name == par.name)
          $("#params_indicator_" + par.name).val(round(parValue, roundDigits));
        break;
  ...
  if (plot && RVtype == "continuous") {
    ...
    if (x2.data.length != 0) {
      ...
      series1 = [
        [x1, 0],
        [x1, fun.apply(null, [x1].concat(args))],
      ];
      series2 = [
        [x2, 0],
        [x2, fun.apply(null, [x2].concat(args))],
      ];
  ...
  plot = $.plot(
    $("#" + this.plotid),
    [{ data: [] }, { data: [] }, { data: series1 }, { data: series2 }, series],
  ...
  function addFill(pos) {
    ...
    for (; i < series.length; i++) {
      if (series[i][0] <= maxx && series[i][0] >= minx)
        newSeries.push(series[i]);
    }

    data[0] = {
      data: newSeries,
      lines: { show: true, fill: true, fillColor: areaFillColor },
    };
  ...
  $("#" + this.plotid).bind("mouseout", function () {
    ...
    if (data[2].data.length == 0) data[0].data = [];
  ...
  $("#" + this.plotid).bind("plotclick", function (event, pos, item) {
    ...
    if (x1.data.length == 0) {
      data[2].data = [
        [pos.x, 0],
        [pos.x, p.density.apply(null, [pos.x].concat(args))],
      ];
  ...
  $("#" + this.plotid).bind("plothover", function (event, pos, item) {
    if (item && RVtype == "discrete") {
      if (previousPoint != item.dataIndex) {
        ...
        showTooltip(item.pageX, item.pageY, funname + "(" + x + ") = " + y);
        if (plotType == "cdf") addCDFLines(item.datapoint[0]);
      }
    } else if (RVtype == "continuous") {
      $("#tooltip").remove();
      var dens = round(
        p.density.apply(null, [pos.x].concat(args)),
        roundDigits
      );
      ...
      if (plotType == "pdf") {
        ...
        showTooltip(
          ...
          "p(" +
            ...
            "<br/>" +
            "Shaded: " +
            round(shaded, roundDigits)
        );
      } else if (plotType == "cdf") {
        addCDFLines(pos.x);
        showTooltip(pos.pageX, pos.pageY, "F(" + x + ") = " + cumdens);
      }
    } else {
      $("#tooltip").remove();
      previousPoint = null;
...
function refTableRow(odd, type) {
  ...
  example.grab(
    new Element("a", {
      ...
      "data-milkbox-size": "width:800,height:507,border-radius:10px",
      "data-milkbox": "dist",
    })
  );
