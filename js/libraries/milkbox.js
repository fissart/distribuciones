...
(function () {
  ...
  this.Milkbox = new Class({
    ...
    options: {
      ...
      resizeDuration: 0.5,
      resizeTransition:
        "sine:in:out" /*function (ex. Transitions.Sine.easeIn) or string (ex. 'bounce:out')*/,
      autoPlay: false,
      autoPlayDelay: 7,
      removeTitle: true,
      autoSize: true,
    ...
    open: function (gallery, index) {
      ...
      var g = instanceOf(gallery, MilkboxGallery)
        ? gallery
    ...
    startAutoPlay: function (opening) {
      var d =
        this.currentGallery.options.autoplay_delay ||
    ...
    preloadFiles: function (preloads) {
      preloads.each(function (fileObj, index) {
        if (
          !this.checkFileType(fileObj, "swf") &&
          !this.checkFileType(fileObj, "html")
        ) {
    ...
    getPageGalleries: function () {
      ...
      names.each(function (name) {
        this.galleries.push(
          new MilkboxGallery($$("a[data-milkbox=" + name + "]"), {
            name: name,
            remove_title: this.options.removeTitle,
          })
        );
    ...
    loadXml: function (xmlfile) {
      var r = new Request({
        ...
        onFailure: function (transport) {
          alert(
            "Milkbox :: loadXml: XML file path error or local Ajax test: please test xml galleries on-line"
          );
        },
      }).send();
    },
    ...
    navAux: function (e, direction) {
      if (e) {
        //called from a button/key event
        this.pauseAutoPlay();
      } else {
      ...
      this.loadFile(this.currentGallery.get_item(i), [
        this.currentGallery.get_item(_i),
      ]);
    },
...
var MilkboxDisplay = new Class({
  ...
  build_html: function () {
    ...
    this.mainbox = new Element("div", {
      ...
      styles: {
        ...
        marginTop: this.options.centered
          ? -(this.options.initialHeight / 2)
          : "",
    ...
    this.bottom = new Element("div#mbox-bottom")
      .setStyle("visibility", "hidden")
      .inject(this.mainbox);
    this.controls = new Element("div#mbox-controls");
    this.caption = new Element("div#mbox-caption", { html: "test" }).setStyle(
      "display",
      "none"
    );

    this.bottom.adopt(
      new Element("div.mbox-reset"),
      this.controls,
      this.caption,
      new Element("div.mbox-reset")
    );
  ...
  prepare_events: function () {
    $$(this.overlay, this.close).addEvent(
      "click",
      function () {
        this.disappear();
      }.bind(this)
  ...
  show_file: function (file, caption, index, length) {
    ...
    if (!file_size.w || !file_size.h) {
      alert(
        "Milkbox error: you must pass size values if the file is swf or html or a free file (openWithFile)"
      );
      return;
    ...
    var filebox_addsize =
      this.filebox.getStyle("border-width").toInt() * 2 +
    ...
    this.caption.setStyle(
      "width",
      final_w -
        caption_adds.paddingRight.toInt() -
        caption_adds.marginRight.toInt()
    );
    $$(this.bottom, this.controls).setStyle(
      "height",
      Math.max(
        this.caption.getDimensions().height,
        this.controls.getComputedSize().totalHeight
      )
    );
    var mainbox_size = this.mainbox.getComputedSize();

    var final_h =
      file_size.h + filebox_addsize + this.bottom.getComputedSize().totalHeight;
  ...
  get_resized_image: function (image) {
    ...
    ratio =
      max_size[check] / i_size[check] <= 1
        ? max_size[check] / i_size[check]
        : 1;
    i_size = Object.map(i_size, function (value) {
      return Math.floor(value * ratio);
    });
    ...
    if (this.options.autoSizeMaxHeight > 0) {
      ratio =
        this.options.autoSizeMaxHeight / i_size.height < 1
          ? this.options.autoSizeMaxHeight / i_size.height
          : 1;
      i_size = Object.map(i_size, function (value) {
        return Math.floor(value * ratio);
      });
  ...
  appear: function () {
    if (!this.options.centered) {
      this.mainbox.setStyle(
        "top",
        window.getScroll().y + this.options.marginTop
      );
    }
  ...
  disappear: function () {
    ...
    $$(this.prev, this.next, this.playpause, this.count).setStyle(
      "display",
      "none"
    );
  ...
  set_mode: function (gallery_type) {
    ...
    switch (gallery_type) {
      case "autoplay":
        ...
        this.controls.setStyle(
          "width",
          close_w + prev_w + next_w + playpause_w + offset
        );
        break;
      case "single":
        $$(this.playpause, this.next, this.prev, this.count).setStyle(
          "display",
          "none"
        );
        ...
        break;
      case "standard":
        $$(this.close, this.next, this.prev, this.count).setStyle(
          "display",
          "block"
        );
    ...
    this.caption.setStyle(
      "margin-right",
      this.controls.getComputedSize().totalWidth
    );
  }, //end set_mode
  ...
  update_count: function (index, length) {
    this.count.set(
      "text",
      index + " " + this.options.imageOfText + " " + length
    );
  },
  ...
  prepare_elements: function () {
    this.items = this.items.map(function (item) {
      ...
      output.caption = output.element
        ? output.element.get("title")
        : item.title;
      if (this.options.remove_title && output.element) {
        output.element.removeProperty("title");
      }

      var size_string = output.element
        ? output.element.get("data-milkbox-size")
        : item.size;
      if (size_string) {
        output.size = Object.map(this.get_item_props(size_string), function (
          value,
          key
        ) {
          return value.toInt();
