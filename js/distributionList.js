  ...
  },
  {
    mean: {
      fun: function (mu, sig2) {
        return jStat.normal.mean(mu, Math.sqrt(sig2));
      },
      display: "\\mu",
    },
    variance: {
      fun: function (mu, sig2) {
        return jStat.normal.variance(mu, Math.sqrt(sig2));
      },
      display: "\\sigma^2",
    },
    median: {
      fun: function (mu, sig2) {
        return jStat.normal.median(mu, Math.sqrt(sig2));
      },
      display: "\\mu",
    },
    mode: {
      fun: function (mu, sig2) {
        return jStat.normal.mode(mu, Math.sqrt(sig2));
      },
      display: "\\mu",
    },
  },
...
var normalMeanPrecision = new distributionParametrization(
  ...
  },
  {
    mean: {
      fun: function (mu, tau) {
        return jStat.normal.mean(mu, 1 / Math.sqrt(tau));
      },
      display: "\\mu",
    },
    variance: {
      fun: function (mu, tau) {
        return jStat.normal.variance(mu, 1 / Math.sqrt(tau));
      },
      display: "\\frac{1}{\\tau}",
    },
    median: {
      fun: function (mu, tau) {
        return jStat.normal.median(mu, 1 / Math.sqrt(tau));
      },
      display: "\\mu",
    },
    mode: {
      fun: function (mu, tau) {
        return jStat.normal.mode(mu, 1 / Math.sqrt(tau));
      },
      display: "\\mu",
    },
  },
...
var normalMeanStandardDeviation = new distributionParametrization(
  ...
  },
  {
    mean: {
      fun: function (mu, sig) {
        return jStat.normal.mean(mu, sig);
      },
      display: "\\mu",
    },
    variance: {
      fun: function (mu, sig) {
        return jStat.normal.variance(mu, sig);
      },
      display: "\\sigma",
    },
    median: {
      fun: function (mu, sig) {
        return jStat.normal.median(mu, sig);
      },
      display: "\\mu",
    },
    mode: {
      fun: function (mu, sig) {
        return jStat.normal.mode(mu, sig);
      },
      display: "\\mu",
    },
  },
...
var gammaShapeRate = new distributionParametrization(
  ...
  {
    mean: {
      fun: function (k, theta) {
        return jStat.gamma.mean(k, 1 / theta);
      },
      display: "\\frac{k}{\\theta}",
    },
    variance: {
      fun: function (k, theta) {
        return jStat.gamma.variance(k, 1 / theta);
      },
      display: "\\frac{k}{\\theta^2}",
    },
    mode: {
      fun: function (k, theta) {
        return jStat.gamma.mode(k, 1 / theta);
      },
...
var studenttLocationScale = new distributionParametrization(
  ...
  "(-\\infty,\\infty)",
  function (x, nu, mu, sig) {
    return jStat.studentt.pdf((x - mu) / sig, nu) / sig;
  },
  "\\frac{\\Gamma\\left(\\frac{\\nu+1}{2}\\right)}{\\sqrt{\\pi\\nu\\sigma^2}\\Gamma\\left(\\frac{\\nu}{2}\\right)}\\left(1+\\frac{(x-\\mu)^2}{\\nu\\sigma^2}\\right)^{-\\frac{\\nu+1}{2}}",
  function (x, nu, mu, sig) {
    return jStat.studentt.cdf((x - mu) / sig, nu);
  },
  null,
  function (nu, mu, sig) {
    return sig > 0 && nu > 0;
...
var lognormalMeanVariance = new distributionParametrization(
  ...
  },
  {
    mean: {
      fun: function (mu, sig2) {
        return jStat.lognormal.mean(mu, Math.sqrt(sig2));
      },
    ...
    median: {
      fun: function (mu, sig2) {
        return jStat.lognormal.median(mu, Math.sqrt(sig2));
      },
      display: "e^\\mu",
    },
    mode: {
      fun: function (mu, sig2) {
        return jStat.lognormal.mode(mu, Math.sqrt(sig2));
      },
...
var weibullShiftShapeScale = new distributionParametrization(
  ...
  function (x0, s, k) {
    return [x0, Number.POSITIVE_INFINITY];
  },
  function (x0, s, k) {
    return [x0, x0 + 10];
  },
  ...
  },
  {
    mean: {
      fun: function (x0, s, k) {
        return jStat.weibull.mean(s, k) + x0;
      },
      display: "x_0 + s\\Gamma\\left(1+\\frac{1}{k}\\right)",
    },
    variance: {
      fun: function (x0, s, k) {
        return jStat.weibull.variance(s, k);
      },
      display:
        "s^2\\Gamma\\left(1+\\frac{2}{k}\\right) - \\left(x_0 + s\\Gamma\\left(1+\\frac{1}{k}\\right)\\right)^2",
    },
    mode: {
      fun: function (x0, s, k) {
        return jStat.weibull.mode(s, k) + x0;
      },
      display: "s\\left(\\frac{k-1}{k}\\right)^{\\frac{1}{k}}, k > 1",
    },
    median: {
      fun: function (x0, s, k) {
        return jStat.weibull.median(s, k) + x0;
      },
...
var weibullShiftShapeRate = new distributionParametrization(
  ...
  {
    mean: {
      fun: function (x0, theta, k) {
        return jStat.weibull.mean(1 / theta, k) + x0;
      },
      display: "x_0 + \\frac{1}{\\theta}\\Gamma\\left(1+\\frac{1}{k}\\right)",
    },
    variance: {
      fun: function (x0, theta, k) {
        return jStat.weibull.variance(1 / theta, k);
      },
      display:
        "\\frac{1}{\\theta^2}\\Gamma\\left(1+\\frac{2}{k}\\right) - \\left(x_0 + \\frac{1}{\\theta}\\Gamma\\left(1+\\frac{1}{k}\\right)\\right)^2",
    },
    mode: {
      fun: function (x0, theta, k) {
        return jStat.weibull.mode(1 / theta, k) + x0;
      },
      display:
        "\\frac{1}{\\theta}\\left(\\frac{k-1}{k}\\right)^{\\frac{1}{k}}, k > 1",
    },
    median: {
      fun: function (x0, theta, k) {
        return jStat.weibull.median(1 / theta, k) + x0;
      },
